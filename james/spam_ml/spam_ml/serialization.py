...
# Names copied mostly from https://github.com/docker/docker-ce/blob/master/components/engine/pkg/namesgenerator/names-generator.go#L9
...
def generate_unique_model_dir(prefix=model_data_cache_path):
    path, name = generate_unique_model_name(prefix)
    os.makedirs(path, exist_ok=True)
    return path, name


def generate_unique_model_name(prefix=model_data_cache_path):
    while True:
...
def get_model_dir(name, prefix=model_data_cache_path):
    return os.path.join(prefix, name)


def generate_model_name():
    l = random.randint(0, len(left) - 1)
    r = random.randint(0, len(right) - 1)

    return f"{left[l]}_{right[r]}"
