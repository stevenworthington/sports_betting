...
def calculate_rolling_stats(
    ...
    sort_cols = (
        [team_col, "SEASON_ID", "GAME_DATE"]
        if stratify_by_season
        else [team_col, "GAME_DATE"]
    )
    ...
    rolling_stats = (
        df_sorted.groupby(group_cols)[filtered_stats_cols]
    ...
    if exclude_initial_games > 0:
        ...
        def set_initial_games_to_nan(group):
            group.loc[
                group.index[:exclude_initial_games], rolled_filtered_stats_cols
            ] = np.nan
            return group

        # apply the function to each group
        rolling_stats = rolling_stats.groupby(
            [team_col] + (["SEASON_ID"] if stratify_by_season else [])
        ).apply(set_initial_games_to_nan)
...
#################################################################################
def calculate_rolling_diff_stats(
    df,
    team_col,
    stats_cols,
    window_size,
    ...
    filtered_stats_cols_both = [
        col for col in stats_cols if col.startswith("HOME_") or col.startswith("AWAY_")
    ]
    ...
    if prefix == "HOME_":
        ...
        rolling_stats = (
            df_sorted.groupby(group_cols)[filtered_stats_cols_both]
    ...
    if prefix == "HOME_":
        rolled_filtered_stats_cols = [
            "ROLLDIFF_" + col for col in filtered_stats_cols_home
        ]

    if prefix == "AWAY_":
        rolled_filtered_stats_cols = [
            "ROLLDIFF_" + col for col in filtered_stats_cols_away
        ]
...
def process_rolling_stats(
    ...
    final_df = final_df.merge(
        rolling_away_stats.drop(["SEASON_ID", "GAME_DATE"], axis=1),
        how="left",
        on="GAME_ID",
    )
...
#################################################################################
def process_rolling_diff_stats(
    df,
    stats_cols,
    target_cols,
    window_size,
    ...
    # calculate rolling stats for home and away teams with optional season stratification and exclusion of initial games
    rolling_home_stats = calculate_rolling_diff_stats(
        df,
        "HOME_TEAM_NAME",
        stats_cols,
        window_size,
    ...
    rolling_away_stats = calculate_rolling_diff_stats(
        df,
        "AWAY_TEAM_NAME",
        stats_cols,
        window_size,
...
def expanding_window_ts_split(
    ...
    if ensure_diversity and target_col is not None:
        ...
        if len(second_class_indices) > 0:
            ...
            raise ValueError(
                "The dataset does not contain enough diversity for the specified target column."
            )
    ...
    while True:
        ...
        if test_end > total_length or (
            expansion_limit is not None
            and total_predictions + (test_end - test_start) > expansion_limit
        ):
            test_end = min(
                total_length,
                (
                    test_start + (expansion_limit - total_predictions)
        ...
        if (
            expansion_limit is not None and total_predictions >= expansion_limit
        ) or test_end >= total_length:
            break
...
def rolling_window_ts_split(
    ...
    if total_length < train_size + test_size + gap_size:
        raise ValueError(
            "The dataset is not large enough for the specified configuration."
        )
    ...
    for start_index in range(0, actual_advancements):
        ...
        yield indices[start_index:train_end_index], indices[
            test_start_index : test_start_index + test_size
        ]
...
def plot_model_performance(
    ...
    assert len(model_outputs) == len(
        observed_values
    ), "Length mismatch between predictions and observed values."
    ...
    plt.plot(
        observed_values.index, model_outputs, label="Predicted", color="red", alpha=0.5
    )
    plt.title(title)
...
#################################################################################
def plot_model_performance_rolling(
    title,
    y_label,
    metric,
    df,
...
def plot_classification_performance(
    ...
    roc_title="ROC Curve",
    overall_title="Classification Performance",
):
    """
    ...
    plt.tight_layout(
        rect=[0, 0.03, 1, 1]
    )  # adjust layout to make room for the overall_title
    plt.show()
